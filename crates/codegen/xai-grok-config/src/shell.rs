...
pub fn detect_windows_shell() -> &'static WindowsShell {
    ...
    CACHED.get_or_init(|| {
        // Explicit override via AMORE_SHELL (primary) / GROK_SHELL (legacy).
        if let Ok(val) = xai_grok_env::var("AMORE_SHELL") {
            match val.trim().to_ascii_lowercase().as_str() {
                "pwsh" => {
                    tracing::info!("Windows shell (AMORE_SHELL/GROK_SHELL override): pwsh");
                    return WindowsShell::Pwsh;
                }
                "powershell" => {
                    tracing::info!(
                        "Windows shell (AMORE_SHELL/GROK_SHELL override): powershell.exe"
                    );
                ...
                "bash" | "gitbash" | "git-bash" => {
                    if let Some(path) = find_git_bash() {
                        tracing::info!(
                            shell = path,
                            "Windows shell (AMORE_SHELL/GROK_SHELL override): Git Bash"
                        );
                        return WindowsShell::GitBash(path);
                    }
                    tracing::warn!(
                        "AMORE_SHELL/GROK_SHELL={val} but Git Bash not found; falling through to auto-detect"
                    );
                }
                "cmd" | "cmd.exe" => {
                    tracing::info!("Windows shell (AMORE_SHELL/GROK_SHELL override): cmd.exe");
                    return WindowsShell::Cmd;
                }
                other => {
                    tracing::warn!(
                        "AMORE_SHELL/GROK_SHELL={other} is not recognized \
...
fn resolve_unix_shell_path(kind: UnixShellKind) -> String {
    ...
    // 1) Explicit override via $AMORE_SHELL / $GROK_SHELL.
    if let Ok(s) = xai_grok_env::var("AMORE_SHELL") {
