...
/// the amore-build GitHub Releases page (URL inserted under the operator's
/// repo-self-URL carve, 2026-07-30).
const MSG_RUN_UPDATE_MANUAL: &str = "This build does not auto-update. Download a newer release from the amore-build GitHub Releases page: https://github.com/vincitamore/amore-build/releases";
...
fn fork_update_notice(current_version: &str) -> String {
    format!(
        "Amore Build - v{current_version}\n\
         Auto-update is disabled in this fork (it must not self-update back to upstream).\n\
         Install newer builds from the amore-build GitHub Releases page."
    )
}
...
#[allow(dead_code)] // fork: the fork-neutral reinstall_hint composes no installer command
...
/// Build a reinstall hint for a known installer type. Fork: every arm
/// resolves to the same fork-neutral hint, because upstream's per-installer
/// commands would reinstall an upstream build.
fn reinstall_hint(_installer: &str, _channel: &str) -> String {
    "Please reinstall from the amore-build GitHub Releases page.".to_string()
}
...
pub async fn run_update_if_available(
    ...
    if auto_update {
        eprintln!(
            "A new version of Amore Build is available: {} -> {}{}",
...
mod tests {
    ...
    fn test_reinstall_hint_npm_is_fork_neutral() {
        ...
        assert!(
            hint.contains("amore-build GitHub Releases"),
    ...
    fn test_reinstall_hint_internal_is_fork_neutral() {
        ...
        assert!(
            ...
            "fork hint composes no channel env: {hint}"
        );
    }
    ...
    async fn test_run_install_script_hard_off_blocks_all_installers() {
        ...
        for installer in ["npm", "gh-release", "internal"] {
            let err = run_install_script(installer, None, &cfg, CliUpdateTrigger::UserCommand)
                .await
    ...
    #[test]
    fn test_reinstall_hint_alpha_channel_is_fork_neutral() {
        // Fork: alpha resolves to the same fork-neutral hint; no channel
        // env, no upstream install URL.
        ...
        assert!(!hint.contains("x.ai"), "must not name an x.ai URL: {hint}");
    }

    #[test]
    fn test_reinstall_hint_enterprise_channel_is_fork_neutral() {
        // Fork: enterprise has no separate bootstrap; same fork-neutral hint.
        ...
        assert!(
            ...
            "must not name the upstream enterprise script: {hint}"
        );
    }
    ...
    fn test_user_facing_constants_are_stable() {
        ...
        assert!(
            MSG_RUN_UPDATE_MANUAL.contains("amore-build GitHub Releases"),
        ...
        assert!(notice.contains("Amore Build"));
        assert!(notice.contains("amore-build GitHub Releases"));
