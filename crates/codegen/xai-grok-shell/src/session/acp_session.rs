...
/// [`state_is_busy`] plus the agent-created obligations the leader's idle-unload decision must honor.
pub(crate) fn state_is_busy_with_obligations(
    state: &State,
...
impl SessionActor {
    ...
    pub(crate) async fn is_busy_live(&self) -> bool {
        ...
        state_is_busy_with_obligations(&state, &scheduled, &background)
    }
}
...
mod is_busy_tests {
    ...
    fn state_is_busy_counts_scheduled_tasks() {
        ...
        assert!(
            !state_is_busy_with_obligations(&state, &none, &[]),
        ...
        assert!(
            state_is_busy_with_obligations(&state, std::slice::from_ref(&scheduled), &[]),
    ...
    fn state_is_busy_counts_background_work() {
        ...
        assert!(
            state_is_busy_with_obligations(&state, &[], std::slice::from_ref(&running)),
        ...
        assert!(
            !state_is_busy_with_obligations(&state, &[], std::slice::from_ref(&done)),
