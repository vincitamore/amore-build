...
/// Join `/compact keep …` text with PreCompact hook additionalContext.
/// Keep-line first, then hook context, separated by a blank line. Empty keep
/// with hook context becomes the hook text; empty hook context is a no-op.
fn merge_precompact_user_context(
    keep: Option<String>,
    hook_contexts: &[String],
) -> Option<String> {
    let hook = if hook_contexts.is_empty() {
        None
    } else {
        Some(hook_contexts.join("\n\n"))
    };
    match (keep, hook) {
        (Some(keep), Some(hook)) if keep.is_empty() => Some(hook),
        (Some(keep), Some(hook)) => Some(format!("{keep}\n\n{hook}")),
        (keep, None) => keep,
        (None, hook) => hook,
    }
}
/// Map PostCompact additionalContext to system-reminder items. Failed, blocked,
/// skipped, and empty contexts produce nothing.
fn postcompact_hook_reminders(
    results: &[xai_grok_hooks::result::HookRunResult],
) -> Vec<ConversationItem> {
    xai_grok_hooks::result::HookRunResult::additional_contexts(results)
        .into_iter()
...
#[cfg(test)]
#[path = "compaction_hook_consume_tests.rs"]
mod compaction_hook_consume_tests;
impl SessionActor {
...
impl SessionActor {
    ...
    async fn run_compact_inner(
        &self,
        mut user_context: Option<String>,
        ...
        let pre_compact_results = self
            .dispatch_hook_collecting(
        ...
        user_context = merge_precompact_user_context(
            user_context,
            &xai_grok_hooks::result::HookRunResult::additional_contexts(&pre_compact_results),
        );
        ...
        let post_compact_results = self
            ...
            .await;
        for reminder in postcompact_hook_reminders(&post_compact_results) {
            self.chat_state_handle.push_user_message(reminder);
        }
        let tokens_after = self.chat_state_handle.get_total_tokens().await;
        {
            let span = tracing::Span::current();
