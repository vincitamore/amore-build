...
impl Default for EndpointsConfig {
    fn default() -> Self {
        Self {
            cli_chat_proxy_base_url: xai_grok_env::var("AMORE_CLI_CHAT_PROXY_BASE_URL").ok(),
...
///
/// Uses the Amore identity layer so mapped `AMORE_*` primaries win over
/// `GROK_*` legacy aliases (e.g. `GROK_DEFAULT_MODEL` / `AMORE_DEFAULT_MODEL`,
/// `GROK_CLI_*` / `AMORE_CLI_*`). Unmapped names are plain env reads.
...
pub struct AgentSelectionConfig {
    ...
    /// Written and advertised by `amore setup` / first-run onboarding.
    #[serde(default, skip_serializing_if = "Option::is_none")]
