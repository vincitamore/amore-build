...
pub(crate) fn discover_hook_source_paths(
    ...
    if let Some(root) = git_root {
        ...
        project.push(classify_grok_hook_source(root.join(".amore").join("hooks")));
...
mod tests {
    ...
    fn discover_includes_amore_hooks_before_grok_hooks() {
        ...
        assert!(
            paths.project.iter().any(|p| matches!(p, HookSourceConfig::Directory(d) if d == &amore)),
            ".amore/hooks must be a project source; got {:?}",
            paths.project
        );
        assert!(
            paths.project.iter().any(|p| matches!(p, HookSourceConfig::Directory(d) if d == &grok)),
            ".grok/hooks must remain a legacy project source; got {:?}",
            paths.project
        );
        let si = paths.project.iter().position(|p| matches!(p, HookSourceConfig::Directory(d) if d == &amore)).unwrap();
        let gi = paths.project.iter().position(|p| matches!(p, HookSourceConfig::Directory(d) if d == &grok)).unwrap();
        assert!(
            si < gi,
