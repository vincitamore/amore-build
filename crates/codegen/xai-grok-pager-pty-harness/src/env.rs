...
fn local_pager_binary_path() -> Result<PathBuf> {
    Ok(target_dir()?
        .join("debug")
        .join(format!("amore{}", std::env::consts::EXE_SUFFIX)))
}
...
fn ensure_local_pager_binary(binary: &std::path::Path) -> Result<()> {
    ...
    let output = cmd
        .output()
        .with_context(|| format!("failed to spawn {cargo} to build amore"))?;

    if !output.status.success() {
        bail!(
            "failed to build amore (exit {:?})\nstdout:\n{}\nstderr:\n{}",
            output.status.code(),
    ...
    if !binary.exists() {
        bail!(
            "amore build completed but binary missing at {}",
            binary.display()
        );
    }
    Ok(())
...
/// 2. `CARGO_BIN_EXE_amore` (set by `cargo test`)
/// 3. Build locally via `cargo build -p xai-grok-pager-bin --bin amore` (the
///    composition-root package that owns the `amore` binary)
pub fn pager_binary() -> Result<PathBuf> {
    ...
    if let Ok(path) = std::env::var("CARGO_BIN_EXE_amore") {
        let p = PathBuf::from(path);
        if p.exists() {
            return Ok(p);
        }
