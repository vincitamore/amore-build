...
//! grok-home (`$AMORE_HOME` primary, `$GROK_HOME` legacy alias, or `<home>/.amore`). Shared by `xai-grok-config`
...
//! - [`default_grok_home`]: the `<home>/.amore` default, ignoring `$AMORE_HOME` / `$GROK_HOME`, so callers can detect an override.
//! - [`resolve_grok_home`]: a fresh, uncached resolve. The env override is read through
//!   `xai_grok_env::var_os("GROK_HOME")` so `$AMORE_HOME` wins.
...
pub enum GrokHomeSource {
    /// A non-empty `$AMORE_HOME` / `$GROK_HOME` override.
    EnvOverride,
    /// `<home>/.amore` derived from the home directory.
    HomeDefault,
}
...
mod tests {
    ...
    fn empty_env_falls_through_to_os_home() {
        ...
        assert_eq!(
            resolved,
            Some((
                dunce::canonicalize(tmp.path()).unwrap().join(".amore"),
