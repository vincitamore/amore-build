...
/// `~/.amore` or `$GROK_HOME`, decided by the resolved home rather than by
...
/// User-facing path under [`grok_home()`], e.g. ``~/.amore/config.toml``.
...
mod tests {
    ...
    fn display_grok_home_prefix_default_install() {
        ...
        assert_eq!(display_grok_home_prefix(), "~/.amore");
    }
