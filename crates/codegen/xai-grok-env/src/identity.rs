//! Selene Build env identity layer: `SELENE_*` primary with `GROK_*` legacy aliases.
//!
//! Semantics (v1):
...
/// Exact suffixes (after `SELENE_` / `GROK_`) that participate in aliasing.
...
/// Prefix wildcards (after `SELENE_` / `GROK_`): every `AUTH_*`, `MDM_*`,
/// `CLI_*` key is aliased.
const PREFIX_SUFFIXES: &[&str] = &["AUTH_", "MDM_", "CLI_"];

/// True when `suffix` (without `SELENE_`/`GROK_` prefix) is in the alias table.
...
/// `(SELENE_…, GROK_…)`. Otherwise `None` (caller should use plain env).
pub fn alias_pair(name: &str) -> Option<(String, String)> {
    let suffix = name
        .strip_prefix("SELENE_")
        .or_else(|| name.strip_prefix("GROK_"))?;
    if !is_mapped_suffix(suffix) {
        return None;
    }
    Some((format!("SELENE_{suffix}"), format!("GROK_{suffix}")))
}

/// Resolve `name` with SELENE primary / GROK legacy semantics when mapped;
...
pub fn alias_table_snapshot() -> Vec<(&'static str, &'static str)> {
    let mut rows: Vec<(&'static str, &'static str)> = EXACT_SUFFIXES
        ...
        .map(|s| {
            ...
            match *s {
                "HOME" => ("SELENE_HOME", "GROK_HOME"),
                "DEFAULT_MODEL" => ("SELENE_DEFAULT_MODEL", "GROK_DEFAULT_MODEL"),
                "SYSTEM_PROMPT_LABEL" => ("SELENE_SYSTEM_PROMPT_LABEL", "GROK_SYSTEM_PROMPT_LABEL"),
                "SHELL" => ("SELENE_SHELL", "GROK_SHELL"),
                "CAMPAIGNS" => ("SELENE_CAMPAIGNS", "GROK_CAMPAIGNS"),
                "CAMPAIGNS_OVERRIDE" => ("SELENE_CAMPAIGNS_OVERRIDE", "GROK_CAMPAIGNS_OVERRIDE"),
                "DEPLOYMENT_CONFIG_CACHE_TTL_SECS" => (
                    "SELENE_DEPLOYMENT_CONFIG_CACHE_TTL_SECS",
                    "GROK_DEPLOYMENT_CONFIG_CACHE_TTL_SECS",
                ),
                "MANAGED_CONFIG_FAIL_CLOSED" => (
                    "SELENE_MANAGED_CONFIG_FAIL_CLOSED",
                    "GROK_MANAGED_CONFIG_FAIL_CLOSED",
                ),
                "AUTH" => ("SELENE_AUTH", "GROK_AUTH"),
                _ => unreachable!("exact suffix not in match: {s}"),
            }
        })
        .collect();
    // Wildcard representatives (the resolver accepts any AUTH_*/MDM_*/CLI_*).
    rows.push(("SELENE_AUTH_PATH", "GROK_AUTH_PATH"));
    rows.push(("SELENE_MDM_DOMAIN", "GROK_MDM_DOMAIN"));
    rows.push(("SELENE_CLI_CHAT_PROXY_BASE_URL", "GROK_CLI_CHAT_PROXY_BASE_URL"));
    rows
}
...
mod tests {
    ...
    fn primary_wins_when_both_present() {
        with_env(
            &[
                ("SELENE_HOME", Some("/selene-home")),
                ("GROK_HOME", Some("/grok-home")),
            ],
            || {
                assert_eq!(var("GROK_HOME").unwrap(), "/selene-home");
                assert_eq!(var("SELENE_HOME").unwrap(), "/selene-home");
            },
        );
    }
    ...
    fn both_absent_is_not_present() {
        with_env(&[("SELENE_SHELL", None), ("GROK_SHELL", None)], || {
            assert!(matches!(var("GROK_SHELL"), Err(VarError::NotPresent)));
            assert!(var_os("SELENE_SHELL").is_none());
        });
    }

    #[test]
    fn unknown_untouched_no_phantom_selene() {
        // GROK_TELEMETRY_ENABLED is intentionally NOT in the alias table.
        with_env(
            &[
                ("SELENE_TELEMETRY_ENABLED", Some("should-not-win")),
            ...
            || {
                assert!(
                    ...
                    "unmapped GROK_* must not invent a SELENE_* alias"
                );
    ...
    fn auth_wildcard_primary_wins() {
        with_env(
            &[
                ("SELENE_AUTH_PATH", Some("/selene/auth.json")),
                ("GROK_AUTH_PATH", Some("/grok/auth.json")),
            ],
            || {
                assert_eq!(var("GROK_AUTH_PATH").unwrap(), "/selene/auth.json");
            },
        );
    }
    ...
    fn cli_wildcard_legacy_fallback() {
        with_env(
            &[
                ("SELENE_CLI_CHAT_PROXY_BASE_URL", None),
    ...
    fn mdm_wildcard_mapped() {
        ...
        with_env(
            &[
                ("SELENE_MDM_PROBE", None),
                ("GROK_MDM_PROBE", Some("mdm-val")),
            ],
            || {
                assert_eq!(var("SELENE_MDM_PROBE").unwrap(), "mdm-val");
            },
        );
    }
    ...
    fn system_prompt_label_aliased() {
        with_env(
            &[
                ("SELENE_SYSTEM_PROMPT_LABEL", Some("Selene From Env")),
    ...
    fn alias_table_covers_required_surface() {
        ...
        for need in [
            "SELENE_HOME",
            "SELENE_DEFAULT_MODEL",
            "SELENE_SYSTEM_PROMPT_LABEL",
            "SELENE_SHELL",
            "SELENE_CAMPAIGNS",
            "SELENE_DEPLOYMENT_CONFIG_CACHE_TTL_SECS",
            "SELENE_MANAGED_CONFIG_FAIL_CLOSED",
            "SELENE_AUTH",
            "SELENE_AUTH_PATH",
            "SELENE_CLI_CHAT_PROXY_BASE_URL",
        ] {
            assert!(
