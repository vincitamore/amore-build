        ...
        tokio::select! {
            ...
            next = l2.next() => match next {
                Some(SamplingEvent::Completed { response, metrics, .. }) => {
                    ...
                    // A zero-output length-stop is an input-overflow signature (the
                    // budget was exhausted before anything was emitted); carry the
                    // model metadata so the shell routes it to compaction. Every
                    // other Length stop goes through the request's `LengthPolicy`.
                    if response.stop_reason == Some(xai_grok_sampling_types::StopReason::Length)
                        && response.empty_reason().is_some()
                    {
                        return AttemptOutcome::Failed {
