...
fn empty_state_markdown(capture_enabled: bool, dream_enabled: bool) -> String {
    ...
    text.push_str(
        "\nAmore Build remembers conventions, decisions, and project facts across sessions so you \
...
While off, Amore isn't reading or saving notes; anything already remembered is kept on disk. \
...
Memory was turned off when Amore Build started, so it can't be turned on here. Anything already \
...
This session reports a reason this version of Amore Build doesn't recognize; press **t** to try \
turning it back on."
        }
    }
}
