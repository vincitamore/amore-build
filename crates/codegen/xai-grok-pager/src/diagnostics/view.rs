...
impl ClipboardRecovery {
    ...
    fn legacy_fix(self) -> Option<&'static str> {
        ...
        fn wrap_ssh_or_minimal() -> &'static str {
            static VALUE: OnceLock<String> = OnceLock::new();
            VALUE.get_or_init(|| {
                format!("{} wrap <ssh command> or /minimal", super::fix::invoked_bin())
            })
        }
        fn wrap_command_or_minimal() -> &'static str {
            static VALUE: OnceLock<String> = OnceLock::new();
            VALUE
                .get_or_init(|| format!("{} wrap <command> or /minimal", super::fix::invoked_bin()))
        }
        fn wrap_or_minimal() -> &'static str {
            static VALUE: OnceLock<String> = OnceLock::new();
            VALUE.get_or_init(|| format!("{} wrap or /minimal", super::fix::invoked_bin()))
        }
        match self {
            Self::Confirmed => None,
            Self::UnverifiedSsh | Self::UnavailableSsh => Some(wrap_ssh_or_minimal()),
            Self::UnverifiedContainer | Self::UnavailableContainer => {
                Some(wrap_command_or_minimal())
            }
            Self::UnverifiedOther => Some(wrap_or_minimal()),
    ...
    match recovery {
        ...
        ClipboardRecovery::UnverifiedSsh => findings.push(manual_finding(
            ...
            format!(
                "When you copy, Grok sends OSC 52 but can't confirm that the outer terminal \
                 accepted it. Each copy is also saved to a backup file; the copy message shows \
                 the path. If paste fails, run `{one_off}` on your local computer or use \
                 `/minimal`. For repeated SSH sessions, run `{bin} doctor fix ssh-wrap` on your \
                 local computer.",
                one_off = crate::diagnostics::ssh_wrap_one_off(),
                bin = crate::diagnostics::fix::invoked_bin(),
            ),
        )),
        ClipboardRecovery::UnverifiedContainer => findings.push(manual_finding(
            ...
            format!(
                "When you copy, Grok sends OSC 52 but can't confirm that the outer terminal \
                 accepted it. Each copy is also saved to a backup file; the copy message shows \
                 the path. If paste fails, start the container command with local `{} wrap \
                 <command>`, or use `/minimal`.",
                crate::diagnostics::fix::invoked_bin(),
            ),
        )),
        ClipboardRecovery::UnverifiedOther => findings.push(manual_finding(
            ...
            format!(
                "Each copy is also saved to a backup file; the copy message shows the path. For \
                 a remote or container command, use local `{} wrap <command>`. You can also use \
        ...
        ClipboardRecovery::UnavailableSsh => findings.push(manual_finding(
            ...
            format!(
                "When you copy, Grok saves the text to the backup file shown in the copy \
                 message. To copy directly, run `{one_off}` on your local computer. For repeated \
                 SSH sessions, run `{bin} doctor fix ssh-wrap` there. You can also use `/copy \
                 <file>` or `/minimal`.",
                one_off = crate::diagnostics::ssh_wrap_one_off(),
                bin = crate::diagnostics::fix::invoked_bin(),
            ),
        ...
        ClipboardRecovery::UnavailableContainer => findings.push(manual_finding(
            ...
            format!(
                "When you copy, Grok saves the text to the backup file shown in the copy \
                 message. Start the container command with local `{} wrap <command>`, use `/copy \
                 <file>`, or use `/minimal`.",
                crate::diagnostics::fix::invoked_bin(),
            ),
        )),
