//! Per-OS spawn chain: open a **new** terminal window running `dioptra dash`.
//!
//! Construction is pure ([`build_spawn_plan`] / [`spawn_plan_for_bin`]); the
//! live spawn ([`spawn_dioptra_dash`]) never panics and never blocks the UI
...
pub fn spawn_dioptra_dash(bin: &Path) -> Result<(), String> {
    ...
    Err(format!(
        "could not open terminal for dioptra dash ({})",
        errors.join("; ")
    ))
}
...
mod tests {
    ...
    fn windows_chain_orders_wt_cmd_powershell() {
        let bin = PathBuf::from(r"C:\tools\dioptra.exe");
        ...
        assert!(plans[0].args.iter().any(|a| a.contains("dioptra")));
    ...
    fn macos_chain_has_terminal_and_iterm() {
        let bin = PathBuf::from("/usr/local/bin/dioptra");
    ...
    fn linux_chain_includes_probe_terminals() {
        let bin = PathBuf::from("/usr/bin/dioptra");
    ...
    fn gnome_terminal_uses_double_dash_separator() {
        let bin = PathBuf::from("/opt/dioptra");
        let plans = build_spawn_plan(&bin, "linux");
        let gnome = plans
            .iter()
        ...
        assert_eq!(gnome.args[1], "/opt/dioptra");
