...
fn reclaim_hint_names_a_sequence_that_frees_space() {
    const AGE: &str = "run `amore worktree gc --max-age 7d --dry-run`";
    const RM: &str = "Remove one with `amore worktree rm --dry-run <path>`";
...
fn symlinked_default_home_keeps_home_label() {
    ...
    std::os::unix::fs::symlink(&real_grok, fake_home.join(".amore")).unwrap();
    let _home = crate::test_util::EnvVarGuard::set("HOME", &fake_home);

    let resolved = dunce::canonicalize(&fake_home).unwrap().join(".amore");
