...
/// RAII plant of a Build session under the process `grok_home()` (OnceLock-
/// cached; do not rely on setting `GROK_HOME` / `AMORE_HOME` mid-process).
///
/// On drop: removes the session directory and, when empty, the encoded cwd
/// parent — so unit tests do not leave harness cwd dirs in the operator's
/// real sessions tree.
struct PlantedLocalSession {
    sess_dir: std::path::PathBuf,
    cwd_dir: std::path::PathBuf,
}

impl PlantedLocalSession {
    /// Disarm the guard and hand back the session dir for callers that clean up themselves.
    fn into_path(self) -> std::path::PathBuf {
        let dir = self.sess_dir.clone();
        std::mem::forget(self);
        dir
    }
}

impl AsRef<std::path::Path> for PlantedLocalSession {
    fn as_ref(&self) -> &std::path::Path {
        &self.sess_dir
    }
}

impl From<&PlantedLocalSession> for std::path::PathBuf {
    fn from(session: &PlantedLocalSession) -> Self {
        session.sess_dir.clone()
    }
}

impl Drop for PlantedLocalSession {
    fn drop(&mut self) {
        let _ = std::fs::remove_dir_all(&self.sess_dir);
        // Only remove the cwd parent when empty (never wipe sibling sessions).
        let _ = std::fs::remove_dir(&self.cwd_dir);
    }
}

/// Plant a Build session under the process `grok_home()`. Prefer holding the
/// returned guard so Drop cleans the session and an empty cwd parent.
fn plant_local_build_session(cwd: &std::path::Path, session_id: &str) -> PlantedLocalSession {
    ...
    PlantedLocalSession { sess_dir, cwd_dir }
}
