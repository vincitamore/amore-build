...
fn auth_required_message(interactive: bool) -> String {
    if interactive {
        "Not signed in. Run `amore login` to authenticate \
         (or `amore login --device-auth` if no browser is available)."
            .to_string()
    } else {
        "Not signed in. To authenticate without a browser, run:\n  \
         amore login --device-auth\n\n\
         Alternatively, set the XAI_API_KEY environment variable \
         or run `amore login` on a machine with a browser."
            .to_string()
    }
}
