...
pub async fn dispatch_non_blocking(
    ...
    for spec in hooks {
        ...
        match result {
            HookRunnerResult::Success { additional_context } => {
                tracing::info!(
                    hook_name = %spec.name,
                    elapsed_ms = elapsed.as_millis() as u64,
...
mod tests {
    ...
    // Shell-shaped (`echo` / `grep` pipelines under `sh -c`): on Windows this asserts the host's
    // shell, not the dispatcher. Gated like the other unix-shaped tests in this file.
    #[cfg(unix)]
    #[tokio::test]
    ...
    /// `hookSpecificOutput.additionalContext` and dispatch surfaces it on
    /// `HookRunResult::Success` for session-context assembly.
    #[tokio::test]
    async fn session_start_additional_context_forwarded() {
        let dir = tempfile::tempdir().unwrap();
        // A script file avoids shell-quote differences between sh / pwsh for inline JSON.
