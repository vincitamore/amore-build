#!/usr/bin/env bun
/**
 * Build the per-OS dioptra release binary via `bun build --compile`.
 *
 * Output: dist/dioptra-{os}-{arch}[.exe]
 *   windows-x64 → dioptra-windows-x64.exe
 *   linux-x64   → dioptra-linux-x64
 *   darwin-arm64→ dioptra-darwin-arm64
 *
 * Embeds: CLI verbs + Bun index daemon (+ @selene/regula).
 * Optional: `--with-dash` also builds dist/dioptra-dash-{os}-{arch}[.exe]
 * (OpenTUI native surface — proven on windows-x64; see PORTING.md §Compile).
 *
 * Usage (from instruments/dioptra):
 ...
 *   bun run scripts/build-compile.ts --outfile path/to/dioptra.exe
 */
...
Options:
  --with-dash              Also build dist/dioptra-dash-{os}-{arch}[.exe]
  --target bun-<os>-<arch> Cross-compile target (Bun --target)
  --outfile path           Override multi-tool outfile

Produces dist/dioptra-{os}-{arch}[.exe] embedding CLI + daemon.
...
const defaultName = `dioptra-${os}-${arch}${ext}`;
...
if (args.withDash) {
  ...
  const dashOut = join(distDir, `dioptra-dash-${os}-${arch}${ext}`);
  ...
  console.log(`[build-compile] dash needs a TTY; set DIOPTRA_DAEMON_BIN=${outfile} for auto-spawn`);
} else {
