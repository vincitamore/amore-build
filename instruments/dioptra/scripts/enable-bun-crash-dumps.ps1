...
# violation) bypasses Bun's JS crash handlers, so the ~/.dioptra/tui-debug.log breadcrumbs and
# ~/.dioptra/tui-crash.log (Bun's stderr trace) narrow WHERE it died — but a full minidump gives
...
$dumpDir = Join-Path $env:USERPROFILE '.dioptra\crashdumps'
