...
shared by the dioptra **CLI** (the agent surface) and the dioptra **TUI** (the operator
...
- **The dioptra daemon** owns *reads* — the index, graph, backlinks, search.
...
In selene, the orient surface is **`AGENTS.md`** (schemas + house doctrine). The
task/inbox/knowledge/reminder domains encode the house AGENTS.md surface — selene admitted `review`/`incubating` to close the
