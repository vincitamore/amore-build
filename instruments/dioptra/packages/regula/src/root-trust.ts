...
//   2. DIOPTRA_ALLOW_FOREIGN_ROOT=1
//   3. path present in ~/.dioptra/allowed-roots.json
//
...
export function foreignRootRemedy(): string {
  return (
    'pass --allow-foreign-root, set DIOPTRA_ALLOW_FOREIGN_ROOT=1, ' +
    'or allow the root (interactive confirm plants ~/.dioptra/allowed-roots.json)'
  );
}

export function allowedRootsPath(homeDir: string = homedir()): string {
  return join(homeDir, '.dioptra', 'allowed-roots.json');
}
...
/**
 * Plant (or reaffirm) a root on the allow list. Creates ~/.dioptra/ as needed.
...
export interface RootTrustOptions {
  ...
  /** Home dir for ~/.dioptra/allowed-roots.json (default os.homedir()). */
  homeDir?: string;
}
...
function envAllowsForeign(env: NodeJS.ProcessEnv): boolean {
  const v = env.DIOPTRA_ALLOW_FOREIGN_ROOT;
...
export function evaluateRootTrust(root: string, opts: RootTrustOptions = {}): RootTrustDecision {
  ...
  if (envAllowsForeign(env)) {
    return {
      ...
      level: 'env',
      root: resolved,
      reason: 'DIOPTRA_ALLOW_FOREIGN_ROOT is set',
    };
  }
  if (isRootAllowed(resolved, homeDir)) {
    return {
      ...
      level: 'allow-list',
      root: resolved,
      reason: 'path is in ~/.dioptra/allowed-roots.json',
    };
  }
...
export async function ensureMutationTrust(
  ...
  if (interactive) {
    if (requireTty && !isTty) {
      throw new RegulaError(
        ...
        foreignRootRefusalMessage(
          decision.root,
          'non-interactive session cannot prompt — pass --allow-foreign-root or set DIOPTRA_ALLOW_FOREIGN_ROOT=1',
        ),
      );
    }
    const homeDir = opts.homeDir ?? homedir();
    ...
    return {
      ...
      level: 'allow-list',
      root: decision.root,
      reason: 'operator confirmed; planted ~/.dioptra/allowed-roots.json',
    };
  }
...
async function defaultConfirm(root: string): Promise<boolean> {
  process.stderr.write(
    `[dioptra] ${resolve(root)} is not a recognized house root (need AGENTS.md|AGENT.md|CLAUDE.md + tasks/).\n` +
      `[dioptra] Allow mutations on this root and remember it in ~/.dioptra/allowed-roots.json? [y/N] `,
  );
