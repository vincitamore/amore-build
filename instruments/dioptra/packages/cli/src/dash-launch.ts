/**
 * Launch the OpenTUI dashboard — the sight. Resolve the TUI entry (sibling
 * package, org-root fallback), wrap the child's stderr in a crash log, and
 * propagate its exit code (annotating native crashes).
 *
 * Consumed by bare `dioptra` / `dioptra dash` (not a global `vitrum` bin).
 * Org-write verbs live under `dioptra <verb>` via
 * `@selene/regula`; the sight names the viewing surface.
 */
...
export async function resolveTuiEntry(): Promise<string> {
  ...
  if (!existsSync(entry)) {
    ...
    if (org) entry = join(org, 'instruments/dioptra/packages/tui/src/index.tsx'); // fallback via org root
  }
  return entry;
}
...
export async function launchDash(passthroughArgs: string[] = []): Promise<never> {
  ...
  // 3221225477) flags a native crash. Pair with ~/.dioptra/tui-debug.log (the dlog breadcrumbs).
  const crashLog = join(homedir(), '.dioptra', 'tui-crash.log');
