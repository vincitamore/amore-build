...
/** Emit a shell completion script that calls back into `dioptra __complete`. */
export function completionScript(shell: 'bash' | 'powershell'): string {
  if (shell === 'bash') {
    return `# dioptra bash completion — add to ~/.bashrc:  source <(dioptra completion bash)
_dioptra_complete() {
  local cur="\${COMP_WORDS[COMP_CWORD]}"
  local prior=("\${COMP_WORDS[@]:1:COMP_CWORD-1}")
  local out
  out="$(dioptra __complete "\${prior[@]}" "$cur" 2>/dev/null)"
  COMPREPLY=( $(compgen -W "$out" -- "$cur") )
}
complete -F _dioptra_complete dioptra
`;
  }
  return `# dioptra PowerShell completion — add to $PROFILE:  dioptra completion powershell | Out-String | Invoke-Expression
Register-ArgumentCompleter -Native -CommandName dioptra -ScriptBlock {
  ...
  (& dioptra __complete @prior $wordToComplete 2>$null) -split "\`n" | Where-Object { $_ } | ForEach-Object {
