...
import { RegulaError, slugify } from '@selene/regula';
...
export function pipelineStatus(orgRoot: string, name: string): Record<string, unknown> {
  ...
  if (!existsSync(manifestPath)) {
    ...
    throw new RegulaError('NOT_FOUND', `No pipeline found: "${name}". Use \`dioptra athanor list\` to see available pipelines.`);
  }
...
export function pipelineResults(
  ...
  if (status !== 'complete' && status !== 'partial') {
    return { name, status, message: `Pipeline is ${status}; results not yet available. Use \`dioptra athanor status\`.` };
  }
 ...
 * follow-up `dioptra athanor status`.
 */
...
export function dispatchRun(plan: RunPlan): Record<string, unknown> {
  ...
  return {
    ...
    pid: child.pid ?? null,
    logPath: plan.logPath,
    message: `Pipeline "${plan.name}" dispatched. Monitor with \`dioptra athanor status ${plan.name}\`.`,
  };
}
