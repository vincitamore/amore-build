...
import * as regula from '@selene/regula';
  ...
  try {
    ...
    throw new regula.RegulaError(
      ...
      `Promoted doc CREATED at ${created.path}, but closing the inbox source '${ref}' FAILED: ${msg}. ` +
        `The new doc is intact; close the source manually (dioptra inbox ${lifecycled ? 'resolve' : 'archive'} ${ref}).`,
    );
  }
...
/** The capability manifest (drives `dioptra commands --json` / `dioptra regula commands --json`). */
