...
/** Locate a Chromium-family browser for headless printing. `$DIOPTRA_BROWSER` overrides. */
export function locateBrowser(): string | null {
  const env = process.env.DIOPTRA_BROWSER;
...
export function exportPdf(doc: PdfDoc, opts: PdfOptions = {}): Promise<PdfResult> {
  return new Promise((resolve) => {
    ...
    if (!browser) {
      resolve({ ok: false, error: 'no Chrome/Edge found (set $DIOPTRA_BROWSER)' });
      return;
    }
