...
// ~/.dioptra/config.json. All readers/writers go through here so independent
// settings (theme, collapsed groups) never clobber each other.

const DIR = join(homedir(), '.dioptra');
const FILE = join(DIR, 'config.json');

export interface DioptraConfig {
  theme?: string;
  /** Collapsed group keys per view (e.g. { tasks: ['paused','complete'] }). */
  collapsed?: Record<string, string[]>;
}

export function readConfig(): DioptraConfig {
  try {
    return JSON.parse(readFileSync(FILE, 'utf8')) as DioptraConfig;
  } catch {
    return {};
  }
}

export function writeConfig(cfg: DioptraConfig): void {
  try {
