...
// Replicates isVitrumDaemonStatus from packages/tui/src/daemon.ts VERBATIM —
...
test('daemon-status satisfies the TUI isDaemonUp shape predicate', async () => {
  ...
  expect(isVitrumDaemonStatus(body)).toBe(true);
});
...
test('daemon-status reports truthful identity: no PTY daemon, bun-daemon service', async () => {
  ...
  expect(info.service).toBe('dioptra-bun-daemon');
