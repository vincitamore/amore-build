...
describe('loadTypedEdges', () => {
  let tmp: string;
  beforeAll(() => {
    tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'dioptra-edges-'));
    fs.mkdirSync(path.join(tmp, 'graph'), { recursive: true });
  });
  afterAll(() => fs.rmSync(tmp, { recursive: true, force: true }));

  test('absent file → []', () => {
    const bare = fs.mkdtempSync(path.join(os.tmpdir(), 'dioptra-noedges-'));
...
describe('edges wired through buildGraph', () => {
  ...
  test('semantic mode swaps wiki links for servable typed edges; nodes unchanged', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'dioptra-edgewire-'));
    fs.mkdirSync(path.join(tmp, 'graph'), { recursive: true });
    fs.writeFileSync(
  ...
  test('wiki (default) never reads edges.jsonl — malformed file is irrelevant', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'dioptra-edgewiki-'));
    fs.mkdirSync(path.join(tmp, 'graph'), { recursive: true });
