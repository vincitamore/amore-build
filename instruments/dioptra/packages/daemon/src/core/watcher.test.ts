...
test('startWatcher: a real add reflects in the live index (create + backlink)', async () => {
  ...
  const prev = process.env.DIOPTRA_WATCH_DEBOUNCE_MS;
  process.env.DIOPTRA_WATCH_DEBOUNCE_MS = '50';
  ...
  try {
    ...
    watcher.stop();
    if (prev === undefined) delete process.env.DIOPTRA_WATCH_DEBOUNCE_MS;
    else process.env.DIOPTRA_WATCH_DEBOUNCE_MS = prev;
  }
});
