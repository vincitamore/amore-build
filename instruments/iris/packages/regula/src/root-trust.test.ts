...
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), 'iris-trust-'));
  fakeHome = mkdtempSync(join(tmpdir(), 'iris-home-'));
});
...
describe('isHouseRoot / evaluateRootTrust', () => {
  ...
  test('IRIS_ALLOW_FOREIGN_ROOT=1 trusts without markers', () => {
    const d = evaluateRootTrust(root, {
      env: { IRIS_ALLOW_FOREIGN_ROOT: '1' },
      homeDir: fakeHome,
    });
    expect(d.trusted).toBe(true);
...
describe('mutation regression cases', () => {
  test('mutation against foreign root refuses with remedy text', () => {
    ...
    try {
      ...
      expect(msg).toContain('IRIS_ALLOW_FOREIGN_ROOT=1');
    }
  });
