...
test('excludes node_modules/.git/.tmp/dist/build/.next/target at ANY depth', () => {
  for (const d of ['node_modules', '.git', '.tmp', 'dist', 'build', '.next', 'target']) {
    ...
    expect(shouldExclude(`instruments/iris/${d}`, true)).toBe(true);
  }
});
...
test('instruments/: only <tool>/{CLAUDE,README}.md + <tool>/design/*.md admitted', () => {
  expect(shouldExclude('instruments/iris/README.md', false)).toBe(false);
  expect(shouldExclude('instruments/iris/CLAUDE.md', false)).toBe(false);
  expect(shouldExclude('instruments/iris/design/spec.md', false)).toBe(false);
  expect(shouldExclude('instruments/iris/package.json', false)).toBe(true);
  expect(shouldExclude('instruments/iris/src/main.rs', false)).toBe(true);
  expect(shouldExclude('instruments/iris/design/sub/deep.md', false)).toBe(true); // len 5
  expect(shouldExclude('instruments/iris/design', true)).toBe(false); // design dir descended
  expect(shouldExclude('instruments/iris/src', true)).toBe(false); // len-3 dir descended (files inside excluded)
  expect(shouldExclude('instruments/iris/src/server', true)).toBe(true); // len-4 dir pruned
});
...
test('walk collects indexed docs and prunes excluded subtrees', () => {
  ...
  write('instruments/iris/README.md');
  write('instruments/iris/design/spec.md');
  write('instruments/iris/src/server/index.md');
  ...
  expect(found.has('instruments/iris/README.md')).toBe(true);
  expect(found.has('instruments/iris/design/spec.md')).toBe(true);
  ...
  expect(found.has('instruments/iris/src/server/index.md')).toBe(false);
