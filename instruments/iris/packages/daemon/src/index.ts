...
Options:
  org_root      Path to the org tree (else $IRIS_ORG_ROOT, else walk up from
                cwd for a dir holding AGENTS.md|AGENT.md|CLAUDE.md beside tasks/).
  --port N      Listen port (default 3853).
...
function parseArgs(argv: string[]): Args {
  const out: Args = { port: 3853, help: false, noWatch: false, allowForeignRoot: false };
...
function main(): void {
  ...
  if (orgRoot === undefined) {
    console.error(
      '[daemon] could not resolve org root (pass a path, set $IRIS_ORG_ROOT, ' +
