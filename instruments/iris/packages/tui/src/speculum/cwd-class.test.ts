...
/** Sample vectors mirrored from the instrument package's cwd-class tests (parity). */
const SAMPLES: { input: string; want: CwdOrigin; note: string }[] = [
  ...
  {
    input: 'C:\\Users\\AlexMoyer\\AppData\\Local\\Temp\\arcus-identity-study\\A-sen-02-r1-FaTNwz',
    want: 'experiment',
...
describe('decodeCwdPath', () => {
  ...
  test('passes through already-decoded paths', () => {
    expect(decodeCwdPath('/tmp/chat-mode-build-refuse-1')).toBe('/tmp/chat-mode-build-refuse-1');
  });
});
...
describe('buildOriginsReport', () => {
  test('counts rows and distinct roots per class', () => {
    const rows = [
      ...
      {
        project_path: 'C:\\Users\\AlexMoyer\\AppData\\Local\\Temp\\arcus-identity-study\\A-sen-01',
        agent: 'primary',
      },
