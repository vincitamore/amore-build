...
beforeEach(() => {
  foreignRoot = mkdtempSync(join(tmpdir(), 'iris-foreign-'));
  houseRoot = mkdtempSync(join(tmpdir(), 'iris-house-'));
...
function runCli(
  ...
  for (const [k, v] of Object.entries(process.env)) {
    if (v !== undefined && k !== 'IRIS_ORG_ROOT' && k !== 'IRIS_ALLOW_FOREIGN_ROOT') {
      merged[k] = v;
    }
  }
...
describe('CLI foreign-root tiered trust (e2e)', () => {
  ...
  test('mutation against foreign root + IRIS_ALLOW_FOREIGN_ROOT=1 proceeds', () => {
    const r = runCli(
      [
        'task',
        'create',
      ...
      { IRIS_ORG_ROOT: foreignRoot, IRIS_ALLOW_FOREIGN_ROOT: '1' },
    );
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toContain('"ok": true');
  });
  ...
  test('read against foreign root works unflagged', () => {
    const r = runCli(['status', '--json'], { IRIS_ORG_ROOT: foreignRoot });
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toContain('"ok": true');
