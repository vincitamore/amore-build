...
test('top-level completions no longer offer dash (glass is bare iris / iris dash; org verbs complete here)', () => {
  const c = resolveCompletions([], '');
...
test('bash completion script targets the iris program name', () => {
  const s = completionScript('bash');
  expect(s).toContain('complete -F _iris_complete iris');
  expect(s).toContain('iris __complete');
  expect(s).not.toContain('regula');
  expect(s).not.toContain('vitrum');
});

test('powershell completion script targets the iris program name', () => {
  const s = completionScript('powershell');
  expect(s).toContain('-CommandName iris');
  expect(s).toContain('& iris __complete');
  expect(s).not.toContain('regula');
  expect(s).not.toContain('vitrum');
});
