 ...
 *
 * Paths may arrive decoded (`sessions.project_path`) or URL-encoded
 * (`%5C`, `%2F`, `%3A` as on-disk dir names under the sessions tree).
 * Classification is a pure string predicate over the decoded form.
 */
...
export function classifyCwd(projectPathOrEncoded: string): CwdOrigin {
  ...
  // --- experiment (operator-run research arms) ---
  if (lower.includes("identity-study") || lower.includes("arcus-model-comparison")) {
    return "experiment";
  }

  // --- unknown: bare /tmp and residual temp that is not harness/experiment ---
  if (lower === "/tmp") return "unknown";
  if (isTempLike(lower)) return "unknown";

  // --- operator: default for non-temp, non-harness paths ---
  return "operator";
}

export type SessionOriginRow = {
  project_path: string;
  /** Reserved for future agent-aware splits; currently unused by the predicate. */
  agent?: string;
};
