#!/usr/bin/env bun
/**
 * Build the per-OS speculum release binary via `bun build --compile`.
 *
 * Output: dist/speculum-{os}-{arch}[.exe]
 *   windows-x64 → speculum-windows-x64.exe
 *   linux-x64   → speculum-linux-x64
 *   darwin-arm64→ speculum-darwin-arm64
 *
 * Embeds: CLI + bun:sqlite store (schema.sql imported as text in store/db.ts).
 *
 * Usage (from instruments/speculum):
...
Produces dist/speculum-{os}-{arch}[.exe] embedding the CLI and sqlite store.`);
  process.exit(0);
}
...
const defaultName = `speculum-${os}-${arch}${ext}`;
...
console.log(`[build-compile] try: ${outfile} --help`);
console.log(`[build-compile]      ${outfile} status --json`);
