...
<repo>/
  ...
  forge/
    README.md
  scripts/                        # orientation sync, test fixtures
    …
  .arcus/
...
| `context/previous-state.md` | Append-only archive of migrated current-state sections | Yours to append on migration | Untouched (disk sha256 == manifest) -> rewrite; your edits always preserved unless --force | Migrate dated current-state sections here verbatim (newest-first); never edit in place. |
