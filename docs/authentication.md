...
Selene Build has **two independent credential rails**. Pick one per model path —
...
| CLI entry | `selene login` → `run_cli_login` | `crates/codegen/xai-grok-pager-bin/src/main.rs` → `xai_grok_shell::auth::run_cli_login` |

```bash
selene login                 # browser / loopback (default)
selene login --oauth         # force OAuth loopback
selene login --device-auth   # device-code (headless / remote)
selene logout                # clear cached session
```
...
**Practical meaning:** one successful `selene login` is enough for first-party
...
### `~/.selene/auth.json` (shape only — never print contents)

Home defaults to **`~/.selene`** (override with `$SELENE_HOME` / legacy
`$GROK_HOME`). Credentials live at `{home}/auth.json` unless
`$SELENE_AUTH_PATH` / `$GROK_AUTH_PATH` points elsewhere.

| Fact | Source |
|------|--------|
| Default home `~/.selene` | `crates/codegen/xai-grok-config/src/paths.rs` (`default_grok_home` / `grok_home`) |
| Path resolution (`SELENE_AUTH` inline JSON, `SELENE_AUTH_PATH`, else `{home}/auth.json`) | `crates/codegen/xai-grok-shell/src/auth/manager.rs` |
...
`$SELENE_HOME`) are fine: they flock `auth.json.lock`, and the loser adopts the
...
restoring a backup of `auth.json`:** delete the bad file (or `selene logout`) and
run **`selene login` again** on that install. Do not “fix” by recopying.

Independent `selene login` on each machine is the supported multi-machine model
...
### Per-model BYOK in `~/.selene/config.toml`
...
### `SELENE_*` primary, `GROK_*` legacy

Wave-3 env identity: for mapped suffixes, **`SELENE_*` wins** when both are set;
...
|---------|--------|-------|
| `SELENE_HOME` | `GROK_HOME` | Config + `auth.json` home |
| `SELENE_AUTH` | `GROK_AUTH` | Inline JSON credential (read-only boot path) |
| `SELENE_AUTH_PATH` | `GROK_AUTH_PATH` | Override path to auth store |
| `SELENE_AUTH_*` | `GROK_AUTH_*` | Wildcard (provider command, TTL, early invalidation, …) |
| `SELENE_CLI_CHAT_PROXY_BASE_URL` | `GROK_CLI_CHAT_PROXY_BASE_URL` | Proxy base override |
...
- Require **an xAI account** that can complete `selene login` and authorize the
...
| Symptom | What to try |
|---------|-------------|
| Auth / 401 loops after copying a home dir | **Do not recopy `auth.json`.** `selene logout` then `selene login` on that machine |
| Headless host, no browser | `selene login --device-auth` |
...
| Want a clean slate | `selene logout` then login or set keys again |
...
- [onboarding.md](onboarding.md) — `selene init` house pack (not credentials)
