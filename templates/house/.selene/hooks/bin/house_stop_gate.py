#!/usr/bin/env python3
"""house stop gate — native Stop hook for Selene Build houses.
...
State lives under ~/.selene/state/stop-gate/ (or $GROK_HOME / $SELENE_HOME).
"""
import json
import os
import re
...
- New craft for a tool/skill we own (gotcha, contract, workflow, design rule) -> its skill under `.selene/skills/<skill>/SKILL.md`
...
def selene_home() -> Path:
    for key in ("SELENE_HOME", "GROK_HOME"):
        val = os.environ.get(key)
        if val:
            return Path(val)
    base = os.environ.get("USERPROFILE") or str(Path.home())
    return Path(base) / ".selene"
...
def is_operator_entry(entry: dict) -> bool:
    ...
    synthetic = (
        ...
        or "[selene stop gate" in lowered
...
def main() -> None:
    ...
    debug = os.environ.get("SELENE_STOP_GATE_DEBUG") == "1"

    def decide(verdict: str) -> None:
        if debug:
            try:
                log_dir = selene_home() / "state" / "stop-gate"
    ...
    state_dir = selene_home() / "state" / "stop-gate"
