...
Manifest at `forge/sessions/{campaign-slug}.manifest.md`, authored **before** the first dispatch — same frontmatter and execution-log shape as forge-master's template with `recipe: auriga` (`forge/README.md` schema: pipeline, recipe, goal, role, layer, triggered-by), plus a `## Work Units` table (unit · wave · tier · child id · owned files · status — the child id column is the fix-round address book, filled at dispatch) in place of the agents-plan YAML, and one added closing section:
...
2. Ownership partition declared in every brief; seam-consumer sweep before wave 1. **The partition IS the isolation**: no per-agent worktree flag in this harness.
3. Waves of 5–7 as parallel task-tool dispatches; read early returns; tune the next wave.
4. Fix-rounds route to the owning builder at report review — resume/re-arm preferred, brief-shaped content — never live-observe a running agent; fresh dispatch only if the owner is dead or the defect crosses boundaries. Commit landed handle+output before any resume (the handle-first stub overwrites them); child ids pinned in the Work Units table at dispatch (§1.5).
