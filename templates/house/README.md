...
Root index for a **Selene Build** house — a continuous working tree for
long-running multi-session agent work. Continuity lives in architecture, not
in any single model's memory.

`selene-build init` plants this tree. Customize [[AGENTS]] for house identity
...
| `.selene/` | Skills and hooks (stop gate + session init) |
