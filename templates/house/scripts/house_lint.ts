...
async function checkTree(root: string, findings: Finding[], notes: string[]): Promise<void> {
  ...
  // Prefer .arcus/ (Arcus Build default); fall back to .grok/ (upstream-grok).
  const hasArcus = existsSync(join(root, ".arcus"));
  const hasGrok = existsSync(join(root, ".grok"));
  if (!hasArcus && !hasGrok) {
    notes.push("orientation-rules-drift: skipped (.arcus/ and .grok/ absent)");
  } else {
